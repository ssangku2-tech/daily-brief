...
**Agenda pipeline (implemented, 2026-08-02):** `.github/workflows/daily-agenda.yml` runs on its own cron (offset a few minutes from the market workflow's, independent workflow so overlap is harmless) and calls `scripts/generate-agenda.js`, which reads the user's Google Calendar (today+tomorrow, KST) and Gmail (unread count + unread-and-important messages) via OAuth2 and writes `agenda/YYYY-MM-DD.json`. Same skip-if-exists-for-the-day convention as `generate.js`. Unlike `generate.js`, **this script never calls the Claude API** — calendar/mail is the user's own already-settled data, so there's nothing to cross-source-verify the way market prices need to be. The same engineering discipline is carried over in an adapted 4-stage shape instead: 0단계 fixes the KST day boundary (today 00:00 → the day after tomorrow 00:00, converted to UTC for the Calendar API — getting this timezone math wrong is this script's equivalent of the market pipeline's "거래일 혼동" bug), 1단계 fixes the collection rules (`allDay` = response has `date` but no `dateTime`; "important mail" = Gmail's own `IMPORTANT` label + unread, not an LLM judgment call), 2단계 assembles the JSON, 3단계 cross-checks filtered counts against the raw API counts and warns on mismatch. OAuth credentials (`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`GOOGLE_REFRESH_TOKEN`) are obtained once via `scripts/setup-google-auth.js` (a local one-time script, not run in CI) and stored as GitHub Actions secrets. Gmail scope is deliberately `gmail.metadata` rather than `gmail.readonly` — it covers everything the script reads (labels, From/Subject headers, snippet) while staying a "sensitive" rather than "restricted" Google scope, which matters because restricted scopes require Google's formal security-assessment process to leave OAuth consent-screen "Testing" status; "Testing" status caps refresh tokens at a 7-day lifetime, which would silently break the daily cron.
