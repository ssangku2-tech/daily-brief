...
`generate.js`'s prompt follows a staged discipline used elsewhere for this user's market briefings: (0) pin down T, the most recently closed US trading session, before researching anything; (1) verify news dates against sources and reject relative-time phrasing ("today"/"this week") in favor of absolute dates; (2) emit a JSON object; (3) self-audit before finalizing. This mirrors the verification rules used in the user's daily Notion market-briefing report — the intent is that this PWA and the Notion report should never disagree on a given day's numbers. As of 2026-08-04 (see below), this discipline governs only news/commentary — the numeric price/changePct fields are no longer part of what Claude verifies.

**Indices/stocks come from a Cloudflare Worker, not from Claude (as of 2026-08-04):** `generate.js` fetches `indices`/`stocks` price and changePct directly from `stock-proxy.ssangku2.workers.dev` (`?symbol=<ticker>` → `{price, prev, currency}`) — the same personal Cloudflare Worker the user's separate "내 주식 현황" portfolio app uses to proxy Yahoo Finance/Naver quotes. Its source is not in this repo; treat its `?symbol=` contract as an external API. This replaced having Claude find-and-verify each price via `web_search`, because the two problems that caused most "확인 실패" cases and all of the historical "거래일 혼동" bugs were solved for free by the Worker: it returns the exact settled number, not a snapshot Claude has to date-disambiguate. It works specifically because `daily.yml`'s cron fires at 06:30 KST, hours after the US regular session closes — at that point Yahoo's `regularMarketPrice` is already frozen to that day's close and no longer moves with after-hours trading, so "T's closing price" and "the price right now" are the same number. If the Worker is unreachable (whole outage) or a single symbol 404s, that item gets `"확인 실패"` exactly as before, and the existing four-times-a-day retry cron is the safety net — there is no separate fallback path. `marketPrompt` now only determines `sessionDate`/`sessionDateLabel`, researches market-moving news, and writes a one-line qualitative comment per watched ticker (`stockNotes`, keyed by ticker) — it does not touch prices at all, which is why its `web_search` budget could drop from 18 to 10 uses.

**Two parallel requests, not one (as of 2026-08-01):** `generate.js` makes two independent Messages API calls via `Promise.all` — `marketPrompt` (sessionDate/news/semiconductor commentary, `max_uses: 10`, `effort: medium`) and `aiNewsPrompt` (Anthropic/Palantier news, `max_uses: 6`, `effort: low`) — then merges the two JSON results (plus the Worker-fetched `indices`/`stocks`, fetched in the same `Promise.all`) into one `brief` object. The two-Claude-request split exists because a single shared `web_search` budget once let the semiconductor cross-verification (which often needed repeated searches when sources disagreed on weekend/stale data) consume the whole budget, silently starving the AI-news section that came last in the prompt — it came back "확인 실패" for both companies with a budget-exhaustion reason, not a genuine no-news finding. Giving each concern its own request with its own budget makes that starvation structurally impossible. Do not re-merge these into a single call without re-solving that starvation problem. (Budgets were trimmed from the original 24/15 to 18/8 to 18/6 on 2026-08-01 for cost, then market dropped again to 10 on 2026-08-04 once it stopped needing numeric cross-verification. aiNews's effort was dropped to `low` since it doesn't need deep numeric reasoning. If "확인 실패" rates rise noticeably in `news`/`aiNews`, raise the relevant budget/effort back first.)
...
- `generate.js`의 프롬프트를 건드릴 때는 0단계(거래일 확정) → 1단계(뉴스 검증 규칙) → 2단계(JSON) → 3단계(자체 감사) 구조를 유지한다. 지수·종목의 price/changePct는 (2026-08-04부터) Claude가 아니라 `fetchLiveIndicesAndStocks()`가 Cloudflare Worker에서 받아오므로, marketPrompt에 다시 숫자 검증·교차확인 지시를 넣지 말 것 — 그건 이미 해결된 "거래일 혼동" 문제를 프롬프트로 되돌리는 것이다. Worker 호출 자체를 건드릴 때는 `WORKER_URL`이 이 레포 소유가 아닌 외부 계약(포트폴리오 앱의 개인 Cloudflare Worker)이라는 점을 유의한다.
