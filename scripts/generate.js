...
2) items: 위 목록에서 읽을 가치가 있는 것만 골라 담는다. 고르지 않은 번호는 아예 넣지 않는다.
   ...
   이건 채워야 할 할당량이 아니라 넘지 말아야 할 상한이다.
   중복 제거가 건수보다 우선한다 — 같은 사건을 다룬 기사가 여럿이면 가장 나은 하나만 남기고,
   그래서 상한에 못 미쳐도 그대로 둔다. 비슷한 기사로 빈자리를 채우지 마라.
   그 밖의 제외 대상: "지금 사야 할까?" 같은 낚시성 제목, 광고·홍보성 기사.
   어떤 종목에 남길 만한 기사가 하나도 없으면 그 종목은 그냥 비워도 된다 —
