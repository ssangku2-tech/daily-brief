...
2) items: 위 목록에서 읽을 가치가 있는 것만 골라 담는다. 고르지 않은 번호는 아예 넣지 않는다.
   섹션별 상한(대괄호 안 라벨 기준): 주요뉴스 4건, 종목별 각 2건, 앤트로픽 3건.
   낚시성·홍보성은 강하게 배제한다: "지금 사야 할까?", "○○를 사야 할 3가지 이유" 같은
   투자 권유형 제목, 광고·홍보성 기사, 근거 없는 전망 칼럼.
   중복 판정은 이 기준 하나로만 한다: 이미 고른 기사를 읽은 사람이 이 기사를 더 읽었을 때
   "새로 알게 되는 사실"이 있는가. 없으면 빼고, 있으면(다른 원인·다른 수치·다른 종목·
   다른 후속 전개) 사건이 같아도 남긴다. 제목 표현만 다르고 전하는 내용이 같으면 뺀다.
   상한을 채우는 것 자체는 목적이 아니다 — 새로 알게 되는 게 없는 기사로 자리를 메우지 마라.
   어떤 종목에 남길 만한 기사가 하나도 없으면(전부 낚시성이면) 그 종목은 비워도 된다 —
   억지로 채우지 마라. 그 종목은 시세만 표시된다.
   회사명 표기: Anthropic 은 "앤트로픽"으로 통일한다.
...
{"summary":"","items":[{"i":0,"ko":""}]}`;
  ...
  try {
    ...
    // 주요뉴스·앤트로픽은 섹션 자체가 비면 화면이 고장난 것처럼 보이므로 최소 1건은 살린다.
    // 종목별 뉴스에는 이 규칙을 적용하지 않는다 — 모델이 그 종목 후보를 전부 버렸다는 건
    // 대개 전부 낚시성이라는 뜻이라, 되살리면 걸러내려던 기사를 (요약도 없이) 되돌려놓게 된다.
    // 남길 게 없는 종목은 시세만 보여주는 편이 낫다.
    for (const b of ['news', 'ai']) {
